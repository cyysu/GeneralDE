...
public:
    ...
    bpg_pkg_t t_bpg_pkg_create(
        size_t capacity = 1024,
        const char * mgr_name = NULL);

    const char * t_bpg_pkg_dump(bpg_pkg_t pkg);

    bpg_pkg_t t_bpg_pkg_build(const char * cfg, const char * mgr_name = NULL);
    bpg_pkg_t t_bpg_pkg_build(cfg_t cfg, const char * mgr_name = NULL);

    void t_bpg_pkg_init(bpg_pkg_t pkg, const char * cfg);
    void t_bpg_pkg_init(bpg_pkg_t pkg, cfg_t cfg);
};
