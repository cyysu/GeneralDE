    ...
    mem_allocrator_t alloc, size_t capacity, int argc, char ** argv)
{
    ...
    void * (*create_fun)(mem_allocrator_t alloc, size_t capacity, void * lib_handler, int argc, char * argv[]);
    ...
    child_context->m_child = create_fun(from->m_alloc, 0, child_context->m_lib_handler, argc, argv);
...
static dir_visit_next_op_t gd_app_load_childs_dir_on_enter(const char * full, const char * base, void * i) {
    ...
    if (cfg_read_file(svr_cfg, cfg_file, cfg_replace, NULL) != 0) {
