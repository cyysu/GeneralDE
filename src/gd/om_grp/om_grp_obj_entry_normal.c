...
uint16_t om_grp_obj_normal_capacity(om_grp_obj_mgr_t mgr, om_grp_obj_t obj, const char * entry) {
    om_grp_entry_meta_t entry_meta = om_grp_entry_meta_find(mgr->m_meta, entry);
    if (entry_meta == NULL) {
        CPE_ERROR(mgr->m_em, "om_grp_obj_normal_capacity: entry %s not exist!", entry);
        return 0;
    }

    return om_grp_obj_normal_capacity_ex(mgr, obj, entry_meta);
}
