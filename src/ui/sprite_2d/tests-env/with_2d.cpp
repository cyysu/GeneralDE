...
UI_SPRITE_2D_RECT with_2d::t_s_2d_rect_build(const char * def) {
    UI_SPRITE_2D_RECT r;
    sscanf(def, "(%f,%f)-(%f,%f)", &r.lt.x, &r.lt.y, &r.rb.x, &r.rb.y);
    return r;
}

const char * with_2d::t_s_2d_rect_dump(UI_SPRITE_2D_RECT const & rect) {
    char buf[64];
...
bool with_2d::t_s_2d_rect_eq(UI_SPRITE_2D_RECT const & l, UI_SPRITE_2D_RECT const & r, float delta) {
    return ui_sprite_2d_rect_eq(&l, &r, delta) ? true : false;
}

bool with_2d::t_s_2d_pair_eq(UI_SPRITE_2D_PAIR const & l, UI_SPRITE_2D_PAIR const & r, float delta) {
