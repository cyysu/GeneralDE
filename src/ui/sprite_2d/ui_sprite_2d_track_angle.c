...
int ui_sprite_2d_track_angle_enter(ui_sprite_fsm_action_t fsm_action, void * ctx) {
    ...
    track_angle->m_total_distance = 0.0f;
...
void ui_sprite_2d_track_angle_update(ui_sprite_fsm_action_t fsm_action, void * ctx, float delta) {
    ...
    struct ui_sprite_2d_track_angle_pos * pre_pos = NULL;;
    ...
    if (track_angle->m_moving_poses_wp != track_angle->m_moving_poses_rp) {
        if (track_angle->m_moving_poses_wp > track_angle->m_moving_poses) {
            pre_pos = track_angle->m_moving_poses_wp - 1;
        }
        else {
            pre_pos = end - 1;
        }
    ...
    track_angle->m_moving_poses_wp->m_pos = transform->m_data.transform.pos;
    track_angle->m_moving_poses_wp->m_distance =
        pre_pos
        ? cpe_math_distance(
            pre_pos->m_pos.x, pre_pos->m_pos.y,
    ...
    track_angle->m_total_distance += track_angle->m_moving_poses_wp->m_distance;

    if (track_angle->m_total_time > track_angle->m_check_span && track_angle->m_total_distance > 0.0f) {
        float angle =
            cpe_math_angle(
        ...
        //printf("xxxxxxxxxxxxxx: update angle %f\n", angle);
    }
    ...
    if (track_angle->m_moving_poses_wp == track_angle->m_moving_poses_rp) {
        ...
        track_angle->m_total_distance -= track_angle->m_moving_poses_rp->m_distance;
