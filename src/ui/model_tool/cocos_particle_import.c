...
int do_cocos_particle_import(ui_data_mgr_t data_mgr, const char * to_particle, const char * plist, const char * pic, error_monitor_t em) {
    ...
    rv = ui_model_import_cocos_particle(ed_mgr, to_particle, plist, pic, em);
    if (rv != 0) {
        CPE_ERROR(em, "import cocos particle fail!");
        goto IMPORT_COMPLETE;
    }
