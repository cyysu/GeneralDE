...
int ui_sprite_anim_camera_touch_set_decorator(ui_sprite_anim_camera_touch_t touch, const char * decorator) {
...
static void ui_sprite_anim_camera_touch_on_begin(void * ctx, ui_sprite_event_t evt) {
    ...
    ui_sprite_anim_camera_updator_stop(&touch->m_updator, camera);

    touch->m_state = ui_sprite_anim_camera_touch_state_idle;

    ui_sprite_anim_camera_screen_world_rect(
        &touch->m_init_camera_rect, &g_full_screen_percent,
        camera, &camera->m_camera_pos, camera->m_camera_scale);

    ui_sprite_fsm_action_sync_update(action, 1);
}

extern void ui_sprite_anim_camera_touch_on_scale(void * ctx, ui_sprite_event_t evt);
...
static void ui_sprite_anim_camera_touch_on_move(void * ctx, ui_sprite_event_t evt) {
    ...
    ui_sprite_world_t world = ui_sprite_entity_world(entity);
    ui_sprite_anim_camera_t camera = ui_sprite_anim_camera_find(world);
    float delta_x = (evt_data->new_screen_pos.x - evt_data->old_screen_pos.x) / camera->m_camera_scale;
    float delta_y = (evt_data->new_screen_pos.y - evt_data->old_screen_pos.y) / camera->m_camera_scale;
    ...
    if (!ui_sprite_anim_camera_updator_is_runing(&touch->m_updator)) {
        /*启动一次新的操作 */
        target_pos.x = camera->m_camera_pos.x - delta_x;
        target_pos.y = camera->m_camera_pos.y - delta_y;
    }
    else {
        /*在上一次操作基础上叠加 */
        target_pos.x = touch->m_updator.m_target_rect.lt.x - delta_x;
        target_pos.y = touch->m_updator.m_target_rect.lt.y - delta_y;
    }

    target_scale = camera->m_camera_scale;
...
static void ui_sprite_anim_camera_touch_on_end(void * ctx, ui_sprite_event_t evt) {
    ...
    if (evt_data->speed_reduce == 0) {
        ...
        if (speed <= 0.001f) {
            ...
            target_pos.x = camera->m_camera_pos.x - speed * duration * cos(radians);
            target_pos.y = camera->m_camera_pos.y - speed * duration * sin(radians);

            ui_sprite_anim_camera_updator_set_max_speed(&touch->m_updator, speed);
            ui_sprite_anim_camera_updator_set_camera(&touch->m_updator, camera, target_pos, camera->m_camera_scale);

            touch->m_state = ui_sprite_anim_camera_touch_state_move_by_speed;
...
static int ui_sprite_anim_camera_touch_enter(ui_sprite_fsm_action_t fsm_action, void * ctx) {
    ...
    if ((ui_sprite_fsm_action_add_event_handler(
        ...
        (ui_sprite_fsm_action_add_event_handler(
             fsm_action, ui_sprite_event_scope_self, 
             "ui_sprite_evt_anim_camera_touch_scale",
             ui_sprite_anim_camera_touch_on_scale, touch) != 0)
        ||
        (ui_sprite_fsm_action_add_event_handler(
            fsm_action, ui_sprite_event_scope_self, 
...
static void ui_sprite_anim_camera_touch_update(ui_sprite_fsm_action_t fsm_action, void * ctx, float delta) {
    ...
    if (touch->m_state == ui_sprite_anim_camera_touch_state_idle) return;
    ...
    ui_sprite_anim_camera_updator_update(&touch->m_updator, camera, delta);

    if (!ui_sprite_anim_camera_updator_is_runing(&touch->m_updator)) {
        if (touch->m_state == ui_sprite_anim_camera_touch_state_move_by_speed) {
            if (ui_sprite_entity_debug(entity)) {
                CPE_INFO(
