...
struct nm_node_type s_nm_node_type_ui_sprite_anim_module = {
    "ui_sprite_anim_module",
    ui_sprite_anim_module_clear
};

static struct {
    const char * name; 
    int (*init)(ui_sprite_anim_module_t module);
    void (*fini)(ui_sprite_anim_module_t module);
} s_auto_reg_products[] = {
    ...
    mem_allocrator_t alloc, const char * name, error_monitor_t em)
{
    ...
    module = (ui_sprite_anim_module_t)nm_node_data(module_node);
    ...
    if (ui_sprite_repository_register_events_by_prefix(
            repo, (LPDRMETALIB)g_metalib_ui_sprite_anim, "ui_sprite_evt") != 0)
    {
    ...
    for(component_pos = 0; component_pos < CPE_ARRAY_SIZE(s_auto_reg_products); ++component_pos) {
        if (s_auto_reg_products[component_pos].init(module) != 0) {
            ...
            ui_sprite_repository_unregister_events_by_prefix(
                repo, (LPDRMETALIB)g_metalib_ui_sprite_anim, "ui_sprite_evt");
    ...
    nm_node_set_type(module_node, &s_nm_node_type_ui_sprite_anim_module);

    return module;
}

static void ui_sprite_anim_module_clear(nm_node_t node) {
    ui_sprite_anim_module_t module;
    int component_pos;

    module = (ui_sprite_anim_module_t)nm_node_data(node);
    ...
    ui_sprite_repository_unregister_events_by_prefix(
        module->m_repo, (LPDRMETALIB)g_metalib_ui_sprite_anim, "ui_sprite_evt");

    mem_buffer_clear(&module->m_dump_buffer);
}

gd_app_context_t ui_sprite_anim_module_app(ui_sprite_anim_module_t module) {
    return module->m_app;
}

void ui_sprite_anim_module_free(ui_sprite_anim_module_t module) {
    nm_node_t module_node;
    assert(module);

	module_node = nm_node_from_data(module);
    if (nm_node_type(module_node) != &s_nm_node_type_ui_sprite_anim_module) return;
    nm_node_free(module_node);
}

ui_sprite_anim_module_t
ui_sprite_anim_module_find(gd_app_context_t app, cpe_hash_string_t name) {
    nm_node_t node;

    node = nm_mgr_find_node(gd_app_nm_mgr(app), name);
    if (node == NULL || nm_node_type(node) != &s_nm_node_type_ui_sprite_anim_module) return NULL;
    return (ui_sprite_anim_module_t)nm_node_data(node);
}

ui_sprite_anim_module_t
ui_sprite_anim_module_find_nc(gd_app_context_t app, const char * name) {
    nm_node_t node;
...
const char * ui_sprite_anim_module_name(ui_sprite_anim_module_t module) {
    return nm_node_name(nm_node_from_data(module));
}

EXPORT_DIRECTIVE
int ui_sprite_anim_module_app_init(gd_app_context_t app, gd_app_module_t module, cfg_t cfg) {
    ui_sprite_anim_module_t ui_sprite_anim_module;
    ...
    ui_sprite_anim_module =
        ui_sprite_anim_module_create(
            ...
            gd_app_module_name(module),
            gd_app_em(app));
    if (ui_sprite_anim_module == NULL) return -1;

    ui_sprite_anim_module->m_debug = cfg_get_int32(cfg, "debug", 0);

    if (ui_sprite_anim_module->m_debug) {
        CPE_INFO(
            gd_app_em(app), "%s: create: done",
            ui_sprite_anim_module_name(ui_sprite_anim_module));
    }
...
EXPORT_DIRECTIVE
void ui_sprite_anim_module_app_fini(gd_app_context_t app, gd_app_module_t module) {
    ui_sprite_anim_module_t ui_sprite_anim_module;

    ui_sprite_anim_module = ui_sprite_anim_module_find_nc(app, gd_app_module_name(module));
    if (ui_sprite_anim_module) {
        ui_sprite_anim_module_free(ui_sprite_anim_module);
    }
}
