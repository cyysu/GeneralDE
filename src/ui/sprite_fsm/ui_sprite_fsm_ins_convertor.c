...
static void ui_sprite_fsm_convertor_on_event(void * ctx, ui_sprite_event_t evt) {
    ...
    if (convertor->m_condition) {
        if (!ui_sprite_entity_calc_bool_with_dft(convertor->m_condition, entity, &input_evt_data_source, 0)) {
    ...
    convert_to = ui_sprite_fsm_action_build_event(convertor->m_action, module->m_alloc, convertor->m_convert_to, &input_evt_data_source);
    if (convert_to == NULL) {
        CPE_ERROR(
        ...
        if (strcmp(ui_sprite_event_handler_process_event(binding->m_handler), event_name) == 0) {
            ...
            if (ui_sprite_entity_debug(entity)) {
                CPE_INFO(
                    ...
                    dr_json_dump_inline(&module->m_dump_buffer, convert_to->data, convert_to->size, convert_to->meta));
            }
