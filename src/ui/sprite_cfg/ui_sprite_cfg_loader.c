...
#define UI_SPRITE_2D_MODULE_LOAD_META(__arg, __name) \
    module-> __arg  = dr_lib_find_meta_by_name((LPDRMETALIB)g_metalib_ui_sprite_2d, __name); \
    assert(module-> __arg)
    ...
    mem_allocrator_t alloc, const char * name, error_monitor_t em)
{
    ...
    if (cpe_hash_table_init(
            ...
            (cpe_hash_cmp_t) ui_sprite_cfg_comp_loader_eq,
    ...
    if (cpe_hash_table_init(
            ...
            (cpe_hash_cmp_t) ui_sprite_cfg_action_loader_eq,
...
int ui_sprite_cfg_loader_app_init(gd_app_context_t app, gd_app_module_t module, cfg_t cfg) {
    ...
    cfg_t cfg_root;
    const char * load_root;

    if ((load_root = cfg_get_string(cfg, "load-root", NULL))) {
        cfg_root = cfg_find_cfg(gd_app_cfg(app), load_root);
        if (cfg_root == NULL) {
            CPE_ERROR(
                gd_app_em(app), "%s: create: load-root %s not exist",
                gd_app_module_name(module), load_root);
            return -1;
        }
    }
	...
	ui_sprite_cfg_loader->m_cfg_root = cfg_root;
