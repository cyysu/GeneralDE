...
static int ui_sprite_cfg_load_resource_track_mgr_metas(ui_sprite_cfg_loader_t loader, ui_sprite_ctrl_track_mgr_t track_mgr, cfg_t cfg);

ui_sprite_world_res_t ui_sprite_cfg_load_resource_track_mgr(void * ctx, ui_sprite_world_t world, cfg_t cfg) {
    ...
    if (ctrl_module == NULL) {
        CPE_ERROR(
            loader->m_em, "%s: create track_mgr resource: ctrl module %s not exist!",
            ui_sprite_cfg_loader_name(loader), ctrl_module_name ? ctrl_module_name : "default");
        return NULL;
    }

    track_mgr = ui_sprite_ctrl_track_mgr_create(ctrl_module, world);
    if (track_mgr == NULL) {
        CPE_ERROR(loader->m_em, "%s: create track_mgr resource: create fail!", ui_sprite_cfg_loader_name(loader));
        return NULL;
    }

    if (ui_sprite_cfg_load_resource_track_mgr_metas(loader, track_mgr, cfg_find_cfg(cfg, "track-types")) != 0) {
...
static int ui_sprite_cfg_load_resource_track_mgr_metas(ui_sprite_cfg_loader_t loader, ui_sprite_ctrl_track_mgr_t track_mgr, cfg_t cfg) {
    ...
    while((meta_cfg = cfg_it_next(&meta_cfg_it))) {
        ...
        if (track_meta == NULL) {
            CPE_ERROR(
                loader->m_em, "%s: create track_mgr resource: track meta %s: create fail!",
                ui_sprite_cfg_loader_name(loader), track_meta_name);
            return -1;
        }
        ...
        while((point_cfg = cfg_it_next(&point_cfg_it))) {
            ...
            if (res == NULL) {
                CPE_ERROR(
                    loader->m_em, "%s: create track_mgr resource: track meta %s: point meta res not set!",
                    ui_sprite_cfg_loader_name(loader), track_meta_name);
                return -1;
            }
