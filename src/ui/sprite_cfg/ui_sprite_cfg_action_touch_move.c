...
ui_sprite_fsm_action_t ui_sprite_cfg_load_action_touch_move(void * ctx, ui_sprite_fsm_state_t fsm_state, const char * name, cfg_t cfg) {
    ...
    if (touch_move == NULL) {
        CPE_ERROR(loader->m_em, "%s: create touch_move action: create fail!", ui_sprite_cfg_loader_name(loader));
        return NULL;
    }
    ...
    if (finger_count <= 0 || finger_count > UI_SPRITE_TOUCH_MAX_FINGER_COUNT) {
        CPE_ERROR(
            loader->m_em, "%s: create touch_move action: finger-count %d error!",
    ...
    if (ui_sprite_touch_move_set_finger_count(touch_move, finger_count) != 0) {
        CPE_ERROR(
            loader->m_em, "%s: create touch_move action: set finger-count %d error!",
            ui_sprite_cfg_loader_name(loader), finger_count);
        ui_sprite_touch_move_free(touch_move);
        return NULL;
    }
    ...
    {
        CPE_ERROR(
            loader->m_em, "%s: create touch_move action: set is-capture error!",
    ...
    {
        CPE_ERROR(
            loader->m_em, "%s: create touch_move action: set is-grab error!",
            ui_sprite_cfg_loader_name(loader));
        ui_sprite_touch_move_free(touch_move);
        return NULL;
    }
    ...
    if (ui_sprite_touch_move_set_on_move(touch_move, cfg_get_string(cfg, "on-move", NULL)) != 0) {
        CPE_ERROR(
            loader->m_em, "%s: create touch_move action: set on-move fail",
            ui_sprite_cfg_loader_name(loader));
        ui_sprite_touch_move_free(touch_move);
        return NULL;
    }
    ...
    if (ui_sprite_touch_move_set_on_end(touch_move, cfg_get_string(cfg, "on-end", NULL)) != 0) {
        CPE_ERROR(
            loader->m_em, "%s: create touch_move action: set on-end fail",
            ui_sprite_cfg_loader_name(loader));
        ui_sprite_touch_move_free(touch_move);
        return NULL;
    }
    ...
    if (ui_sprite_touch_move_set_on_cancel(touch_move, cfg_get_string(cfg, "on-cancel", NULL)) != 0) {
        CPE_ERROR(
            loader->m_em, "%s: create touch_move action: set on-cancel fail",
            ui_sprite_cfg_loader_name(loader));
        ui_sprite_touch_move_free(touch_move);
        return NULL;
    }
