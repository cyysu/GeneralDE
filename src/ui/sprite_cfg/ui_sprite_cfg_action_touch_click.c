...
ui_sprite_fsm_action_t ui_sprite_cfg_load_action_touch_click(void * ctx, ui_sprite_fsm_state_t fsm_state, const char * name, cfg_t cfg) {
    ...
    if (touch_click == NULL) {
        CPE_ERROR(loader->m_em, "%s: create touch_click action: create fail!", ui_sprite_cfg_loader_name(loader));
        return NULL;
    }
    ...
    if (finger_count <= 0 || finger_count > UI_SPRITE_TOUCH_MAX_FINGER_COUNT) {
        CPE_ERROR(
            loader->m_em, "%s: create touch_click action: finger-count %d error!",
    ...
    if (ui_sprite_touch_click_set_finger_count(touch_click, finger_count) != 0) {
        CPE_ERROR(
            loader->m_em, "%s: create touch_click action: set finger-count %d error!",
            ui_sprite_cfg_loader_name(loader), finger_count);
        ui_sprite_touch_click_free(touch_click);
        return NULL;
    }
    ...
    {
        CPE_ERROR(
            loader->m_em, "%s: create touch_click action: set is-grab error!",
    ...
    if (ui_sprite_touch_click_set_on_click_down(touch_click, cfg_get_string(cfg, "on-click-down", NULL)) != 0) {
        CPE_ERROR(
            loader->m_em, "%s: create touch_click action: set on-click-down fail",
            ui_sprite_cfg_loader_name(loader));
        ui_sprite_touch_click_free(touch_click);
        return NULL;
    }
    ...
    if (ui_sprite_touch_click_set_on_click_up(touch_click, cfg_get_string(cfg, "on-click-up", NULL)) != 0) {
        CPE_ERROR(
            loader->m_em, "%s: create touch_click action: set on-click-up fail",
            ui_sprite_cfg_loader_name(loader));
        ui_sprite_touch_click_free(touch_click);
        return NULL;
    }
