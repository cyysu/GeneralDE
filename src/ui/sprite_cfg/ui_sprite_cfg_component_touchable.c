...
int ui_sprite_cfg_load_component_touchable(void * ctx, ui_sprite_component_t component, cfg_t cfg) {
    ...
    while((child_cfg = cfg_it_next(&child_cfg_it))) {
        ...
        if (str_shape == NULL) {
            CPE_ERROR(
                loader->m_em, "%s: entity %d(%s): create Touchable: box: shape type not configure",
        ...
        if (strcmp(str_shape, "box") == 0) {
            ...
            if (base_pos == NULL) {
                CPE_ERROR(
                    loader->m_em, "%s: entity %d(%s): create Touchable: circle: base pos not conrigured!",
                    ui_sprite_cfg_loader_name(loader),
                    ui_sprite_entity_id(entity), ui_sprite_entity_name(entity));
                return -1;
            }
            ...
            if (shape.data.circle.base_pos == 0) {
                CPE_ERROR(
                    loader->m_em, "%s: entity %d(%s): create Touchable: circle: base pos %s error!",
                    ui_sprite_cfg_loader_name(loader),
            ...
            if (shape.data.circle.radius <= 0.0f) {
                CPE_ERROR(
                    loader->m_em, "%s: entity %d(%s): create Touchable: circle: radius %f error!",
                    ui_sprite_cfg_loader_name(loader),
            ...
            CPE_ERROR(
                loader->m_em, "%s: entity %d(%s): create Touchable: box: shape type %s unknown",
                ui_sprite_cfg_loader_name(loader),
