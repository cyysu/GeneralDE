...
static int ui_sprite_cfg_load_component_turntable_track(
    ui_sprite_cfg_loader_t loader, UI_SPRITE_CTRL_TURNTABLE_DEF * def, cfg_t cfg);

int ui_sprite_cfg_load_component_turntable(void * ctx, ui_sprite_component_t component, cfg_t cfg) {
    ...
    if ((track_cfg = cfg_find_cfg(cfg, "track"))) {
        if (ui_sprite_cfg_load_component_turntable_track(loader, &def, track_cfg) != 0) return -1;
    }
    ...
    if ((slots_cfg = cfg_find_cfg(cfg, "slots"))) {
        ...
        while((slot_cfg = cfg_it_next(&slots_it))) {
            if (def.slot_count + 1 > CPE_ARRAY_SIZE(def.slots)) {
                CPE_ERROR(
                    loader->m_em, "%s: create turntable component: slots overflow!!",
            ...
            if (cfg_try_as_float(slot_cfg, &def.slots[def.slot_count++]) != 0) {
                CPE_ERROR(
                    loader->m_em, "%s: create turntable component: load slot, format error!",
                    ui_sprite_cfg_loader_name(loader));
                return -1;
            }
        }
    ...
    if (ui_sprite_ctrl_turntable_set_def(turntable, &def) != 0) {
        CPE_ERROR(
            loader->m_em, "%s: create turntable component: set def fail!",
            ui_sprite_cfg_loader_name(loader));
        return -1;
    }
    ...
    ui_sprite_cfg_loader_t loader, UI_SPRITE_CTRL_TURNTABLE_DEF * def, cfg_t cfg)
{
    ...
    if (type == NULL) {
        CPE_ERROR(
            loader->m_em, "%s: create turntable component: load track, type not configured!",
            ui_sprite_cfg_loader_name(loader));
        return -1;
    }
    ...
    if (strcmp(type, "circle") == 0) {
        ...
        if (def->track_def.circle.radius <= 0.0f) {
            CPE_ERROR(
                loader->m_em, "%s: create turntable component: load track: circle: radius %f is error!",
                ui_sprite_cfg_loader_name(loader), def->track_def.circle.radius);
            return -1;
        }
    }
        ...
        if (def->track_def.ellipse.radius_x <= 0.0f || def->track_def.ellipse.radius_y <= 0.0f) {
            CPE_ERROR(
                loader->m_em, "%s: create turntable component: load track: ellipse: radius (%f,%f) is error!",
                ui_sprite_cfg_loader_name(loader), def->track_def.ellipse.radius_x, def->track_def.ellipse.radius_x);
            return -1;
        }
    }
    else {
        CPE_ERROR(
            loader->m_em, "%s: create turntable component: load track, type %s unknown!",
            ui_sprite_cfg_loader_name(loader), type);
        return -1;
    }
