...
#include "ui/model_ed/ui_ed_src.h"
...
ui_model_import_cocos_particle_carete_mod(ui_ed_obj_t emitter_obj, uint8_t mod_type, error_monitor_t em) {
    ui_ed_obj_t mod_obj;
    UI_PARTICLE_MOD * mod_data;

    mod_obj = ui_ed_obj_new(emitter_obj, ui_ed_obj_particle_mod);
    if (mod_obj == NULL) {
...
static int ui_model_import_cocos_particle_size(ui_ed_obj_t emitter_obj, cfg_t cfg, error_monitor_t em) {
    UI_PARTICLE_MOD * mod;
    ...
    mod = ui_model_import_cocos_particle_carete_mod(emitter_obj, ui_particle_mod_type_size_uniform_seed, em);
    ...
    mod = ui_model_import_cocos_particle_carete_mod(emitter_obj, ui_particle_mod_type_size_uniform_over_life, em);
    if (mod == NULL) return -1;
...
static int ui_model_import_cocos_particle_color(ui_ed_obj_t emitter_obj, cfg_t cfg, error_monitor_t em) {
    UI_PARTICLE_MOD * mod;
    ...
    mod = ui_model_import_cocos_particle_carete_mod(emitter_obj, ui_particle_mod_type_color_over_life, em);
    if (mod == NULL) return -1;
...
static int ui_model_import_cocos_particle_speed(ui_ed_obj_t emitter_obj, cfg_t cfg, error_monitor_t em) {
    UI_PARTICLE_MOD * mod;
    float speed = cfg_get_float(cfg, "speed", 0.0f);
    float speed_d = cfg_get_float(cfg, "speedVariance", 0.0f);
    float angle = cfg_get_float(cfg, "angle", 0.0f) * M_PI / 180.0f;
    ...
    mod = ui_model_import_cocos_particle_carete_mod(emitter_obj, ui_particle_mod_type_velocity_seed, em);
    if (mod == NULL) return -1;
...
static int ui_model_import_cocos_particle_accel(ui_ed_obj_t emitter_obj, cfg_t cfg, error_monitor_t em) {
    UI_PARTICLE_MOD * mod;
    float gravity_x = cfg_get_float(cfg, "gravityx", 0.0f);
    float gravity_y = cfg_get_float(cfg, "gravityy", 0.0f);

    mod = ui_model_import_cocos_particle_carete_mod(emitter_obj, ui_particle_mod_type_accel_seed, em);
    if (mod == NULL) return -1;
...
static int ui_model_import_cocos_particle_location(ui_ed_obj_t emitter_obj, cfg_t cfg, error_monitor_t em) {
    UI_PARTICLE_MOD * mod;
    float diff_x = cfg_get_float(cfg, "sourcePositionVariancex", 0.0f);
    float diff_y = cfg_get_float(cfg, "sourcePositionVariancey", 0.0f);

    mod = ui_model_import_cocos_particle_carete_mod(emitter_obj, ui_particle_mod_type_location_seed, em);
    if (mod == NULL) return -1;
...
static int ui_model_import_cocos_particle_lifetime(ui_ed_obj_t emitter_obj, cfg_t cfg, error_monitor_t em) {
    UI_PARTICLE_MOD * mod;
    ...
    mod = ui_model_import_cocos_particle_carete_mod(emitter_obj, ui_particle_mod_type_lifetime_seed, em);
    if (mod == NULL) return -1;
...
static int ui_model_import_cocos_particle_rotate(ui_ed_obj_t emitter_obj, cfg_t cfg, error_monitor_t em) {
    UI_PARTICLE_MOD * mod;
    ...
    mod = ui_model_import_cocos_particle_carete_mod(emitter_obj, ui_particle_mod_type_rotation2d_seed, em);
    if (mod == NULL) return -1;
...
int ui_model_import_cocos_particle(
    ...
    const char * plist, const char * pic, error_monitor_t em)
{
    ...
    /*创建Emitter*/
    emitter_obj = ui_ed_obj_new(src_obj, ui_ed_obj_particle_emitter);
    if (emitter_obj == NULL) {
    ...
    strncpy(emitter_data->texture, pic, sizeof(emitter_data->texture));

    if (ui_model_import_cocos_particle_lifetime(emitter_obj, root_cfg, em) != 0
        || ui_model_import_cocos_particle_location(emitter_obj, root_cfg, em) != 0
        || ui_model_import_cocos_particle_accel(emitter_obj, root_cfg, em) != 0
        || ui_model_import_cocos_particle_size(emitter_obj, root_cfg, em) != 0
        || ui_model_import_cocos_particle_color(emitter_obj, root_cfg, em) != 0
        || ui_model_import_cocos_particle_speed(emitter_obj, root_cfg, em) != 0
        || ui_model_import_cocos_particle_rotate(emitter_obj, root_cfg, em) != 0
        )
    {
        cfg_free(root_cfg);
        return -1;
