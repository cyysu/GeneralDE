...
ui_sprite_attr_monitor_create(
    ...
    const char * input_attrs, ui_sprite_entity_t entity,
...
void ui_sprite_attr_monitor_free(const ui_sprite_world_t world, ui_sprite_attr_monitor_t monitor) {
    ui_sprite_repository_t repo = world->m_repo;
    ...
    ui_sprite_attr_monitor_t monitor, const char * name)
{
    ...
    if (attr == NULL) {
        CPE_ERROR(
            repo->m_em, "entity %d(%s): create attr monitor binding to %s: attr not exist!",
