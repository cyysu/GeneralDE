...
static void ui_sprite_entity_notify_attr_updated_one(ui_sprite_entity_t entity, const char * path);
    ...
    ui_sprite_entity_t entity, char * attr, int attr_capacity, int attr_len, LPDRMETAENTRY entry)
{
    ...
    for(i = 0; i < entry_num; ++i) {
        ...
        if (dr_entry_type(child_entry) > CPE_DR_TYPE_COMPOSITE) {
            ...
            ui_sprite_entity_notify_composite_attr_updated(
...
static void ui_sprite_entity_notify_attr_updated_one(ui_sprite_entity_t entity, const char * path) {
    ...
    if (dr_entry_type(attr->m_entry) > CPE_DR_TYPE_COMPOSITE) {
        ...
        memcpy(buf, path, len);
        ui_sprite_entity_notify_composite_attr_updated(entity, buf, CPE_ARRAY_SIZE(buf), len, attr->m_entry);
    }
}
