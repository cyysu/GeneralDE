...
static int ui_ed_src_do_load(ui_ed_src_t ed_src, error_monitor_t em);
...
ui_ed_src_t ui_ed_src_create_i(ui_ed_mgr_t ed_mgr, ui_data_src_t data_src, ui_ed_src_state_t state) {
    ...
    ed_src->m_root_obj =
        ui_ed_obj_create_i(ed_src, NULL, ui_ed_obj_src, ed_src, &ed_src->m_data, sizeof(ed_src->m_data));
...
ui_ed_src_t ui_ed_src_find_by_path(ui_ed_mgr_t ed_mgr, const char * path) {
    ui_data_src_t data_src;

    data_src = ui_data_src_child_find_by_path(ui_data_mgr_src_root(ed_mgr->m_data_mgr), path);
    if (data_src == NULL) return NULL;
...
ui_ed_src_t ui_ed_src_find_by_data(ui_ed_mgr_t ed_mgr, ui_data_src_t data_src) {
    ...
    if (ed_src == NULL) {
        ...
        if (ui_data_src_is_loaded(ed_src->m_data_src)) {
            int r = ui_ed_src_do_load(ed_src, ed_mgr->m_em);
            if (r) {
...
ui_ed_src_t ui_ed_src_new(ui_ed_mgr_t ed_mgr, const char * path, ui_data_src_type_t type) {
    ...
    if (ui_ed_src_do_load(ed_src, ed_mgr->m_em) != 0) {
...
int ui_ed_src_init(ui_ed_src_t ed_src) {
    ...
    if ((rv = ui_ed_src_do_load(ed_src, ed_src->m_ed_mgr->m_em)) != 0) {
...
int ui_ed_src_load(ui_ed_src_t ed_src, error_monitor_t em) {
    ...
    return ui_ed_src_do_load(ed_src, em);
}
...
ui_ed_src_t ui_ed_src_check_create(ui_ed_mgr_t ed_mgr, const char * path, ui_data_src_type_t type) {
    ...
    ed_src = ui_ed_src_find_by_path(ed_mgr, path);
    if (ed_src == NULL) {
        ...
        if (ui_ed_src_type(ed_src) != type) {
            CPE_ERROR(
                ed_mgr->m_em, "check create src at %s: type error, require %d, but is %d!",
                path, type, ui_ed_src_type(ed_src));
            return NULL;
        }
...
extern int ui_ed_src_load_particle(ui_ed_src_t src);
static int ui_ed_src_do_load(ui_ed_src_t ed_src, error_monitor_t em) {
    int (*load_fun)(ui_ed_src_t src);

    switch(ui_data_src_type(ed_src->m_data_src)) {
    case ui_data_src_type_module:
        load_fun = ui_ed_src_load_module;
        break;
    case ui_data_src_type_sprite:
        load_fun = ui_ed_src_load_sprite;
        break;
    case ui_data_src_type_action:
        load_fun = ui_ed_src_load_action;
        break;
    case ui_data_src_type_layout:
        load_fun = ui_ed_src_load_layout;
        break;
    case ui_data_src_type_particle:
        load_fun = ui_ed_src_load_particle;
        break;
    default:
        CPE_ERROR(em, "ed load src: not support src type %d", ui_data_src_type(ed_src->m_data_src));
        ui_data_src_unload(ed_src->m_data_src);
        return -1;
    }

    if (load_fun(ed_src) != 0) {
        CPE_ERROR(em, "ed load src: load objs fail");
        ui_data_src_unload(ed_src->m_data_src);
        return -1;
    }
