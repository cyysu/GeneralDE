...
void Package::appendData(const char *name, int64_t millis) {
    if (mongo_pkg_append_date(*this, name, millis) != 0) {
        throw ::std::runtime_error("Usf::Mongo::Package: append date fail!");
    }
}

void Package::appendTimeS(const char *name, time_t secs) {
    if (mongo_pkg_append_time_t(*this, name, secs) != 0) {
        throw ::std::runtime_error("Usf::Mongo::Package: append time fail!");
    }
}
