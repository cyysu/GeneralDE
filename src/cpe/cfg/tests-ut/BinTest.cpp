...
cfg_t BinTest::build_by_bin(cfg_t cfg) {
    ...
    int write_r = cfg_write_bin_to_buffer(&buffer, cfg, t_em());
    EXPECT_LT(0, write_r);
    ...
    EXPECT_EQ(0, cfg_read_bin_with_name(m_root, name, mem_buffer_make_continuous(&buffer, 0), mem_buffer_size(&buffer), t_em()));
