...
static void * cfg_write_bin_append(struct cfg_bin_write_ctx * ctx, size_t capacity);
static int32_t cfg_write_bin_add_string(struct cfg_bin_write_ctx * ctx, const char *);
static int cfg_write_bin_ctx_init(struct cfg_bin_write_ctx * ctx, void * result, size_t result_capacity, mem_buffer_t result_buffer, error_monitor_t em);
static void cfg_write_bin_ctx_fini(struct cfg_bin_write_ctx * ctx);
static int cfg_write_bin_build_strings(struct cfg_bin_write_ctx * ctx);

#define CFG_WRITE_BIN_APPEND_TYPE(__type) \
    tmp_buf = cfg_write_bin_append(&ctx, sizeof(uint8_t));  \
    if (tmp_buf == NULL) goto WRITE_ERROR; \
    *(uint8_t*)tmp_buf = __type

#define CFG_WRITE_BIN_APPEND_STRING(__str) \
    tmp_str = cfg_write_bin_add_string(&ctx, __str); \
    if (tmp_str < 0) goto WRITE_ERROR; \
    tmp_buf = cfg_write_bin_append(&ctx, sizeof(tmp_str));  \
    if (tmp_buf == NULL) goto WRITE_ERROR; \
...
#define CFG_WRITE_BIN_APPEND_BASIC(__type, __cfg)   \
    ...
    case CPE_CFG_TYPE_FLOAT:                            \
        tmp_buf = cfg_write_bin_append(&ctx, 4);                    \
        if (tmp_buf == NULL) goto WRITE_ERROR;                  \
    ...
    case CPE_CFG_TYPE_DOUBLE:                           \
        tmp_buf = cfg_write_bin_append(&ctx, 8);                    \
        if (tmp_buf == NULL) goto WRITE_ERROR;                  \
        CPE_COPY_HTON64(tmp_buf, cfg_data(__cfg));              \
        break;                                          \
    case CPE_CFG_TYPE_STRING:                           \
        tmp_str = cfg_write_bin_add_string(&ctx, (const char *)cfg_data(__cfg)); \
        if (tmp_str < 0) goto WRITE_ERROR; \
        tmp_buf = cfg_write_bin_append(&ctx, sizeof(tmp_str));  \
        if (tmp_buf == NULL) goto WRITE_ERROR; \
        CPE_COPY_HTON32(tmp_buf, &tmp_str);             \
        break;                                          \
    default:                                            \
        CPE_ERROR(ctx.m_em, "cfg_write_bin: append_basic: unknown basic type %d!", (__type)); \
        goto WRITE_ERROR;                                               \
    }

static int cfg_write_bin_i(void * result, size_t result_capacity, mem_buffer_t result_buffer, cfg_t cfg, error_monitor_t em) {
    ...
    if (cfg_write_bin_ctx_init(&ctx, result, result_capacity, result_buffer, em) != 0) return -1;

    if (cfg_write_bin_append(&ctx, sizeof(struct cfg_format_bin_head)) == NULL) goto WRITE_ERROR;

    cur_type = cfg_type(cfg);
    CFG_WRITE_BIN_APPEND_TYPE(cur_type);
    if (cur_type == CPE_CFG_TYPE_SEQUENCE || cur_type == CPE_CFG_TYPE_STRUCT) {
        processStack[0].m_size_pos = ctx.m_used_size;
        processStack[0].m_parent = cfg;
        cfg_it_init(&processStack[0].m_cfg_it, cfg);
        stackPos = 0;

        if (cfg_write_bin_append(&ctx, sizeof(uint32_t)) == NULL) goto WRITE_ERROR;
    }
    else {
    ...
    WRITE_GO_NEXT:
            ...
            while((cur_cfg = cfg_it_next(&cur_stack->m_cfg_it))) {
                ...
                if (cfg_type_is_value(cur_type)) {
                    ...
                    if (stackPos + 1 >= CPE_ARRAY_SIZE(processStack)) {
                        CPE_ERROR(ctx.m_em, "cfg_write_bin: max level reached!");
                        goto WRITE_ERROR;
                    }
                    else {
                        ...
                        new_stack->m_size_pos = ctx.m_used_size;
        ...
        case CPE_CFG_TYPE_STRUCT:
            ...
            break;
        default:
            CPE_ERROR(ctx.m_em, "cfg_write_bin: not support write basic value!");
            goto  WRITE_ERROR;
        }

        *(uint32_t*)(ctx.m_output_buf + cur_stack->m_size_pos) = (uint32_t)(ctx.m_used_size - sizeof(struct cfg_format_bin_head));
    ...
    /*构造string表 */
    if (cfg_write_bin_build_strings(&ctx) != 0) goto WRITE_ERROR;

    /*填写head */
    head = (struct cfg_format_bin_head *)ctx.m_output_buf;
    assert(head);

    head->m_magic = CFG_FORMAT_BIN_MATIC;
    head->m_data_start = sizeof(struct cfg_format_bin_head);
    head->m_data_capacity = data_total_len - head->m_data_start;
    head->m_strpool_start = data_total_len;
    head->m_strpool_capacity = ctx.m_used_size - head->m_strpool_start;

    /*清理 */
    data_total_len = ctx.m_used_size;
    cfg_write_bin_ctx_fini(&ctx);
...
int cfg_write_bin(void * result, size_t result_capacity, cfg_t cfg, error_monitor_t em) {
    ...
    if (em) {
        ...
        r = cfg_write_bin_i(result, result_capacity, NULL, cfg, em);
        CPE_DEF_ERROR_MONITOR_REMOVE(logError, em);
    }
    else {
        CPE_DEF_ERROR_MONITOR(logError, cpe_error_save_last_errno, &err);
        r = cfg_write_bin_i(NULL, 0, result, cfg, &logError);
    }

    return r < 0 ? err : r;
}

int cfg_write_bin_to_buffer(struct mem_buffer * result, cfg_t cfg, error_monitor_t em) {
    int err = 0;
    int r;

    if (em) {
        CPE_DEF_ERROR_MONITOR_ADD(logError, em, cpe_error_save_last_errno, &err);
        r = cfg_write_bin_i(NULL, 0, result, cfg, em);
        CPE_DEF_ERROR_MONITOR_REMOVE(logError, em);
    }
    else {
        CPE_DEF_ERROR_MONITOR(logError, cpe_error_save_last_errno, &err);
...
static void * cfg_write_bin_append(struct cfg_bin_write_ctx * ctx, size_t require) {
    size_t total_size;
    void * r;
    ...
    if (total_size > ctx->m_output_capacity) {
        if (ctx->m_output_alloc) {
            uint32_t new_capacity = ctx->m_output_capacity < 1024 ? 1024 : ctx->m_output_capacity * 2;
...
static int32_t cfg_write_bin_add_string(struct cfg_bin_write_ctx * ctx, const char * str) {
    ...
    if (ele == NULL) {
        ...
        if (ele == NULL) {
            CPE_ERROR(ctx->m_em, "cfg_write_bin: add_string: alloc string_ele fail!");
            return -1;
        }
...
static int cfg_write_bin_build_strings(struct cfg_bin_write_ctx * ctx) {
    ...
    TAILQ_FOREACH(ele, &ctx->m_string_list, m_next) {
        uint32_t len = strlen(ele->m_str) + 1;
        void * buf = cfg_write_bin_append(ctx, len);
        if (buf == NULL) {
            CPE_ERROR(ctx->m_em, "cfg_write_bin: build_strings: alloc buf fail, len=%d", len);
            return -1;
        }
...
static int cfg_write_bin_ctx_init(
    struct cfg_bin_write_ctx * ctx,
    void * result, size_t result_capacity, mem_buffer_t result_buffer, error_monitor_t em)
{
    ...
    if (cpe_hash_table_init(
            ...
            (cpe_hash_cmp_t) cfg_bin_write_str_ele_eq,
            CPE_HASH_OBJ2ENTRY(cfg_bin_write_str_ele, m_hh),
            -1) != 0)
    {
        CPE_ERROR(em, "cfg_write_bin: init string hash fail!");
        return -1;
    }
