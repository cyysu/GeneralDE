...
set_svr_svr_type_t
set_svr_svr_type_create(set_svr_t svr, const char * svr_type_name) {
    cfg_t svr_cfg;
    ...
    svr_cfg = cfg_find_cfg(cfg_find_cfg(gd_app_cfg(svr->m_app), "svr_types"), svr_type_name);
    if (svr_cfg == NULL) {
        CPE_ERROR(
            svr->m_em, "%s: create type of svr type %s: no config!",
            set_svr_name(svr), svr_type_name);
    }
