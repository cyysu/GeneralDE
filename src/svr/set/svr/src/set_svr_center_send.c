...
RESIZE_AND_TRY_AGAIN:
    ...
    if (center->m_svr->m_debug) {
        struct write_stream_buffer stream = CPE_WRITE_STREAM_BUFFER_INITIALIZER(&center->m_svr->m_dump_buffer_body);
        mem_buffer_clear_data(&center->m_svr->m_dump_buffer_body);
        ...
        CPE_INFO(
            ...
            (const char *)mem_buffer_make_continuous(&center->m_svr->m_dump_buffer_body, 0));
    }
