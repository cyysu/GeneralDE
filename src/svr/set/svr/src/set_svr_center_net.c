    ...
    while(center->m_rb) {
        ...
        if (pkg) {
            if (center->m_svr->m_debug) {
                struct write_stream_buffer stream = CPE_WRITE_STREAM_BUFFER_INITIALIZER(&svr->m_dump_buffer_body);
                mem_buffer_clear_data(&svr->m_dump_buffer_body);

                dr_json_print((write_stream_t)&stream, pkg, pkg_decode_len, center->m_pkg_meta, DR_JSON_PRINT_MINIMIZE, NULL);
                ...
                CPE_INFO(
                    ...
                    (const char *)mem_buffer_make_continuous(&svr->m_dump_buffer_body, 0));
            }
