    ...
    error_monitor_t em)
{
    ...
    VERSION_SVR_LOAD_META(m_meta_res_query_update, "svr_version_res_query_update");
...
int version_svr_parse_version(SVR_VERSION_VERSION * version, const char * str) {
    ...
    version->parts[0] = atoi(p);

    p = sep + 1;
    sep = strchr(p, '.');
    if (sep == NULL) return -1;
    version->parts[1] = atoi(p);

    p = sep + 1;
    version->parts[2] = atoi(p);
...
int version_svr_parse_version_cmp(SVR_VERSION_VERSION const * l, SVR_VERSION_VERSION const * r) {
    ...
    for(i = 0; i < CPE_ARRAY_SIZE(r->parts); ++i) {
        if (l->parts[i] == r->parts[i]) continue;
        return l->parts[i] < r->parts[i] ? -1 : 1;
    }
