...
int main(int argc, char * argv[]) {
    ...
    struct arg_int *  shm = arg_int0(NULL, "use-shm", NULL,    "using shm");
    struct arg_int *  reset = arg_int0(NULL, "shm-reset", NULL,    "reset shm data");
    struct arg_int *  record_count = arg_int1(NULL, "record-count", NULL,    "max record count");
    struct arg_dbl *  bucket_ratio = arg_dbl0(NULL, "bucket-ratio", NULL,    "record hash table bucket count per record, default: 1.5");
    struct arg_end  * run_end = arg_end(20);
    void* run_argtable[] = { run, run_pidfile, run_root, run_app_id, shm, reset, record_count, bucket_ratio, run_end };
    int run_nerrors;
