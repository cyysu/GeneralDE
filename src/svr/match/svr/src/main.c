...
int tool_shm_init(int shm_key, int shm_size, int force, error_monitor_t em) {
    LPDRMETA meta = dr_lib_find_meta_by_name((LPDRMETALIB)g_metalib_svr_match_pro, "svr_room_cli_record");
    if (meta == NULL) {
...
PRINT_HELP:
    ...
    printf("usage 6: %s ", argv[0]); arg_print_syntax(stdout, start_argtable, "\n");
    printf("usage 7: %s ", argv[0]); arg_print_syntax(stdout, stop_argtable, "\n");
    printf("usage 8: %s ", argv[0]); arg_print_syntax(stdout, common_argtable, "\n");
