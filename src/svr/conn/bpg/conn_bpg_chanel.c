...
#include "conn_bpg_internal_ops.h"

static void conn_bpg_chanel_clear(nm_node_t node);

struct nm_node_type s_nm_node_type_conn_bpg_chanel = {
    "svr_conn_bpg_chanel",
    conn_bpg_chanel_clear
};
...
conn_bpg_chanel_create(
    ...
    error_monitor_t em)
{
    struct conn_bpg_chanel * mgr;
    nm_node_t mgr_node;

    assert(app);

    mgr_node = nm_group_create(gd_app_nm_mgr(app), name, sizeof(struct conn_bpg_chanel));
    if (mgr_node == NULL) return NULL;

    mgr = (conn_bpg_chanel_t)nm_node_data(mgr_node);
    ...
    nm_node_set_type(mgr_node, &s_nm_node_type_conn_bpg_chanel);

    return mgr;
}

static void conn_bpg_chanel_clear(nm_node_t node) {
    conn_bpg_chanel_t mgr;
    mgr = (conn_bpg_chanel_t)nm_node_data(node);
    ...
    if (mgr->m_conn_head) {
        conn_cli_pkg_free(mgr->m_conn_head);
        mgr->m_conn_head = NULL;
    }
}

gd_app_context_t conn_bpg_chanel_app(conn_bpg_chanel_t mgr) {
    return mgr->m_app;
}
...
void conn_bpg_chanel_free(conn_bpg_chanel_t mgr) {
    ...
    if (nm_node_type(mgr_node) != &s_nm_node_type_conn_bpg_chanel) return;
...
conn_bpg_chanel_find(gd_app_context_t app, cpe_hash_string_t name) {
    ...
    if (node == NULL || nm_node_type(node) != &s_nm_node_type_conn_bpg_chanel) return NULL;
    return (conn_bpg_chanel_t)nm_node_data(node);
}
...
cpe_hash_string_t
conn_bpg_chanel_name_hs(conn_bpg_chanel_t mgr) {
    return nm_node_name_hs(nm_node_from_data(mgr));
}

int conn_bpg_chanel_set_incoming_dispatch_to(conn_bpg_chanel_t sp, const char * incoming_dispatch_to) {
...
cpe_hash_string_t conn_bpg_chanel_incoming_dispatch_to(conn_bpg_chanel_t sp) {
    return sp->m_incoming_dispatch_to;
}

int conn_bpg_chanel_set_incoming_recv_at(conn_bpg_chanel_t sp, const char * incoming_recv_at) {
    char name_buf[128];

    snprintf(name_buf, sizeof(name_buf), "%s.incoming-recv-rsp", conn_bpg_chanel_name(sp));
    ...
    dp_rsp_set_processor(sp->m_incoming_recv_at, conn_bpg_chanel_incoming_recv, sp);

    if (dp_rsp_bind_string(sp->m_incoming_recv_at, incoming_recv_at, sp->m_em) != 0) {
        CPE_ERROR(
            sp->m_em, "%s: set incoming_recv_at: bind to %s fail!",
            conn_bpg_chanel_name(sp), incoming_recv_at);
...
int conn_bpg_chanel_set_outgoing_dispatch_to(conn_bpg_chanel_t sp, const char * outgoing_dispatch_to) {
...
cpe_hash_string_t conn_bpg_chanel_outgoing_dispatch_to(conn_bpg_chanel_t sp) {
    return sp->m_outgoing_dispatch_to;
}

int conn_bpg_chanel_set_outgoing_recv_at(conn_bpg_chanel_t sp, const char * outgoing_recv_at) {
    char name_buf[128];

    snprintf(name_buf, sizeof(name_buf), "%s.outgoing-recv-rsp", conn_bpg_chanel_name(sp));
    ...
    dp_rsp_set_processor(sp->m_outgoing_recv_at, conn_bpg_chanel_outgoing_recv, sp);

    if (dp_rsp_bind_string(sp->m_outgoing_recv_at, outgoing_recv_at, sp->m_em) != 0) {
        CPE_ERROR(
            sp->m_em, "%s: set outgoing_recv_at: bind to %s fail!",
            conn_bpg_chanel_name(sp), outgoing_recv_at);
