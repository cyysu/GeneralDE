...
DECODE_PKG:
    ...
    if (chanel->m_incoming_buf == NULL) {
        ...
        if (chanel->m_incoming_buf == NULL) {
            CPE_ERROR(chanel->m_em, "%s: incoming_recv: alloc incoming_buf fail, size=%d!", conn_bpg_chanel_name(chanel), (int)cur_data_size);
            return -1;
        }
    }
    ...
    if (decode_rv == dr_cvt_result_not_enough_output) {
        if (cur_data_size < chanel->m_pkg_max_size) {
            ...
            CPE_ERROR(chanel->m_em, "%s: incoming_recv: decode pkg require buf too big, cur_data_size=%d!", conn_bpg_chanel_name(chanel), (int)cur_data_size);
            return -1;
        }
    }
    else if (decode_rv != 0) {
        CPE_ERROR(chanel->m_em, "%s: incoming_recv: decode pkg fail, rv=%d!", conn_bpg_chanel_name(chanel), (int)decode_rv);
        return -1;
    }
...
int conn_bpg_chanel_outgoing_recv(dp_req_t req, void * ctx, error_monitor_t em) {
    ...
    if (chanel->m_conn_head == NULL) {
        chanel->m_conn_head = conn_cli_pkg_create(chanel->m_conn_cli);
        if (chanel->m_conn_head == NULL) {
            CPE_ERROR(chanel->m_em, "%s: outgoing_recv: conn_head create fail!", conn_bpg_chanel_name(chanel));
            return -1;
        }
    }
...
ENCODE_PKG:
    ...
    if (chanel->m_outgoing_buf == NULL) {
        ...
        if (chanel->m_outgoing_buf == NULL) {
            CPE_ERROR(chanel->m_em, "%s: outgoing_recv: alloc outgoing_buf fail, size=%d!", conn_bpg_chanel_name(chanel), (int)cur_data_size);
            return -1;
        }
    }
    ...
    if (encode_rv == dr_cvt_result_not_enough_output) {
        if (cur_data_size < chanel->m_pkg_max_size) {
            ...
            CPE_ERROR(chanel->m_em, "%s: outgoing_recv: encode pkg require buf too big, cur_data_size=%d!", conn_bpg_chanel_name(chanel), (int)cur_data_size);
            return -1;
        }
    }
    else if (encode_rv != 0) {
        CPE_ERROR(chanel->m_em, "%s: outgoing_recv: encode pkg fail, rv=%d!", conn_bpg_chanel_name(chanel), encode_rv);
        return -1;
    }

    conn_cli_pkg_set_sn(chanel->m_conn_head, bpg_pkg_sn(req));
    dp_req_set_parent(conn_cli_pkg_to_dp_req(chanel->m_conn_head), chanel->m_outgoing_buf);
