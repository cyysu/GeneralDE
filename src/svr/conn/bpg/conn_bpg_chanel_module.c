...
EXPORT_DIRECTIVE
int conn_bpg_chanel_app_init(gd_app_context_t app, gd_app_module_t module, cfg_t cfg) {
    conn_bpg_chanel_t sp;
    ...
    conn_cli = conn_cli_find_nc(app, cfg_get_string(cfg, "conn-cli", NULL));
    if (conn_cli == NULL) {
        CPE_ERROR(
    ...
    sp = conn_bpg_chanel_create(
        app, gd_app_module_name(module),
        bpg_pkg_manage, conn_cli, gd_app_alloc(app), gd_app_em(app));
    if (sp == NULL) return -1;

    sp->m_debug = cfg_get_int8(cfg, "debug", sp->m_debug);

    if (conn_bpg_chanel_set_incoming_recv_at(sp, incoming_recv_at) != 0) {
        CPE_ERROR(
            gd_app_em(app), "%s: create: set incoming-recv-at %s fail!",
            gd_app_module_name(module), incoming_recv_at);
        conn_bpg_chanel_free(sp);
        return -1;
    }

    if (conn_bpg_chanel_set_incoming_dispatch_to(sp, incoming_dispatch_to) != 0) {
        CPE_ERROR(
    ...
    if (conn_bpg_chanel_set_outgoing_recv_at(sp, outgoing_recv_at) != 0) {
        CPE_ERROR(
    ...
    if (conn_bpg_chanel_set_outgoing_dispatch_to(sp, outgoing_dispatch_to) != 0) {
        CPE_ERROR(
...
EXPORT_DIRECTIVE
void conn_bpg_chanel_app_fini(gd_app_context_t app, gd_app_module_t module) {
    conn_bpg_chanel_t conn_bpg_chanel;

    conn_bpg_chanel = conn_bpg_chanel_find_nc(app, gd_app_module_name(module));
    if (conn_bpg_chanel) {
        conn_bpg_chanel_free(conn_bpg_chanel);
    }
}
