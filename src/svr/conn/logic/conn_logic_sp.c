...
#include "svr/conn/logic/conn_logic_sp.h"
#include "conn_logic_internal_ops.h"

static void conn_logic_sp_clear(nm_node_t node);
extern char g_metalib_conn_logic_pkg_info[];

struct nm_node_type s_nm_node_type_conn_logic_sp = {
    "svr_conn_logic_sp",
    conn_logic_sp_clear
};

conn_logic_sp_t
conn_logic_sp_create(
    ...
    error_monitor_t em)
{
    struct conn_logic_sp * mgr;
    nm_node_t mgr_node;

    assert(app);

    mgr_node = nm_group_create(gd_app_nm_mgr(app), name, sizeof(struct conn_logic_sp));
    if (mgr_node == NULL) return NULL;

    mgr = (conn_logic_sp_t)nm_node_data(mgr_node);
    ...
    nm_node_set_type(mgr_node, &s_nm_node_type_conn_logic_sp);
...
static void conn_logic_sp_clear(nm_node_t node) {
    conn_logic_sp_t mgr;
    mgr = (conn_logic_sp_t)nm_node_data(node);
...
void conn_logic_sp_free(conn_logic_sp_t mgr) {
    ...
    if (nm_node_type(mgr_node) != &s_nm_node_type_conn_logic_sp) return;
    nm_node_free(mgr_node);
}

conn_logic_sp_t
conn_logic_sp_find(gd_app_context_t app, cpe_hash_string_t name) {
    nm_node_t node;

    node = nm_mgr_find_node(gd_app_nm_mgr(app), name);
    if (node == NULL || nm_node_type(node) != &s_nm_node_type_conn_logic_sp) return NULL;
    return (conn_logic_sp_t)nm_node_data(node);
}

conn_logic_sp_t
conn_logic_sp_find_nc(gd_app_context_t app, const char * name) {
    nm_node_t node;

    if (name == NULL) name = "conn_logic_sp";
...
cpe_hash_string_t
conn_logic_sp_name_hs(conn_logic_sp_t mgr) {
...
static int conn_logic_sp_incoming_recv(dp_req_t req, void * ctx, error_monitor_t em) {
    conn_logic_sp_t sp = ctx;
    conn_cli_pkg_t pkg = conn_cli_pkg_find(req);
    logic_require_t require;
    logic_data_t data;

    require = logic_require_queue_remove_get(sp->m_require_queue, conn_cli_pkg_sn(pkg));
    if (require == NULL) {
        CPE_ERROR(
            sp->m_em, "%s: receive response of %d: require not exist, ignore!",
            conn_logic_sp_name(sp), conn_cli_pkg_sn(pkg));
        return -1;
    }
...
int conn_logic_sp_set_incoming_recv_at(conn_logic_sp_t sp, const char * incoming_recv_at) {
    ...
    snprintf(name_buf, sizeof(name_buf), "%s.incoming-recv-rsp", conn_logic_sp_name(sp));
    ...
    logic_require_t require)
{
    ...
    body = conn_logic_sp_outgoing_buf(sp, data_size);
    if (body == NULL) {
        CPE_ERROR(sp->m_em, "%s: send_request: get body buf fail!", conn_logic_sp_name(sp));
        return -1;
    }

    pkg = conn_cli_pkg_find(body);
    assert(pkg);
    ...
    if (require) {
        conn_cli_pkg_set_sn(pkg, logic_require_id(require));
        if (logic_require_queue_add(sp->m_require_queue, logic_require_id(require)) != 0) {
            CPE_ERROR(sp->m_em, "%s: send_request: add require fail!", conn_logic_sp_name(sp));
    ...
    if (dp_dispatch_by_string(sp->m_outgoing_dispatch_to, body, sp->m_em) != 0) {
        CPE_ERROR(
            ...
            conn_logic_sp_name(sp), cpe_hs_data(sp->m_outgoing_dispatch_to));
        dp_req_set_buf(body, NULL, 0);
        dp_req_set_meta(body, NULL);
        return -1;
    }
...
dp_req_t conn_logic_sp_outgoing_buf(conn_logic_sp_t sp, size_t capacity) {
    if (sp->m_outgoing_pkg == NULL) {
        sp->m_outgoing_pkg = conn_cli_pkg_create(sp->m_cli);
        if (sp->m_outgoing_pkg == NULL) {
            CPE_ERROR(sp->m_em, "%s: crate outgoing pkg buf fail!", conn_logic_sp_name(sp));
            return NULL;
        }
    }
    ...
    if (sp->m_outgoing_body == NULL) {
        ...
        dp_req_set_parent(sp->m_outgoing_body, conn_cli_pkg_to_dp_req(sp->m_outgoing_pkg));
    }

    conn_cli_pkg_init(sp->m_outgoing_pkg);
