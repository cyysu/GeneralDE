...
EXPORT_DIRECTIVE
int conn_logic_sp_app_init(gd_app_context_t app, gd_app_module_t module, cfg_t cfg) {
    conn_logic_sp_t sp;
    ...
    if (conn_logic_sp_set_outgoing_dispatch_to(sp, outgoing_dispatch_to) != 0) {
        CPE_ERROR(
            gd_app_em(app), "%s: create: set outgoing-send-to %s fail!",
            gd_app_module_name(module), outgoing_dispatch_to);
        conn_logic_sp_free(sp);
        return -1;
    }
    
    if (conn_logic_sp_set_incoming_recv_at(sp, incoming_recv_at) != 0) {
        CPE_ERROR(
...
EXPORT_DIRECTIVE
void conn_logic_sp_app_fini(gd_app_context_t app, gd_app_module_t module) {
    conn_logic_sp_t conn_logic_sp;

    conn_logic_sp = conn_logic_sp_find_nc(app, gd_app_module_name(module));
    if (conn_logic_sp) {
        conn_logic_sp_free(conn_logic_sp);
    }
}
