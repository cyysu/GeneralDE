    ...
    if (data) {
        ...
        if (encode_size < 0) {
            if (encode_size == dr_code_error_not_enough_output) {
                if (curent_pkg_size >= UINT16_MAX) {
                    CPE_ERROR(
                        svr->m_em, "%s: conn(conn_id=%d, fd=%d, user_id="FMT_UINT64_T"): send: not enough encode buf, buf size is %d!",
                        conn_svr_name(svr), conn->m_conn_id, conn->m_fd, conn->m_user_id, (int)curent_pkg_size);
                    ...
                    curent_pkg_size *= 2;
                    if (curent_pkg_size > UINT16_MAX) curent_pkg_size = UINT16_MAX;
