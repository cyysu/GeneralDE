            ...
            if (bytes > 0) {
                if (svr->m_debug >= 2) {
                    CPE_INFO(
                        ...
                        conn_svr_name(svr), conn->m_conn_id, conn->m_fd, conn->m_user_id, bytes);
                }
                ...
                default:
                    ...
                    CPE_ERROR(
                        ...
                        conn_svr_name(svr), conn->m_conn_id, conn->m_fd, conn->m_user_id, cpe_sock_errno(), cpe_sock_errstr(cpe_sock_errno()));
                    conn_svr_conn_free(conn);
                    return;
                }
            }
        ...
        while(conn->m_wb) {
            ...
            if (bytes > 0) {
                ...
                CPE_ERROR(
                    ...
                    conn_svr_name(svr), conn->m_conn_id, conn->m_fd, conn->m_user_id, err, cpe_sock_errstr(err));
                conn_svr_conn_free(conn);
                return;
            }
        }
    ...
    while(conn->m_rb) {
        ...
        if (pkg_data_len < sizeof(SVR_CONN_NET_REQ_HEAD)) {
            CPE_ERROR(
                ...
                conn_svr_name(svr), conn->m_conn_id, conn->m_fd, conn->m_user_id, pkg_data_len);
            conn_svr_conn_free(conn);
            return -1;
        }
        ...
        if (ss_pkg == NULL) {
            ...
            if (ss_pkg == NULL) {
                CPE_ERROR(
                    ...
                    conn_svr_name(svr), conn->m_conn_id, conn->m_fd, conn->m_user_id, ss_pkg_len);
                conn_svr_conn_free(conn);
                return -1;
            }
        ...
        set_pkg_set_to_svr(ss_pkg_head, to_svr_type, 1);
        ...
        if (backend == NULL) {
            CPE_ERROR(
                ...
                conn_svr_name(svr), conn->m_conn_id, conn->m_fd, conn->m_user_id, to_svr_type);
        ...
        default:
            CPE_ERROR(
                ...
                conn_svr_name(svr), conn->m_conn_id, conn->m_fd, conn->m_user_id, to_svr_type, backend->m_safe_policy);
            conn->m_rb = ringbuffer_yield(svr->m_ringbuf, conn->m_rb, pkg_data_len);
