...
conn_svr_conn_create(conn_svr_t svr, int fd, uint32_t conn_id) {
    ...
    conn = mem_alloc(svr->m_alloc, sizeof(struct conn_svr_conn));
    if (conn == NULL) {
    ...
    if (conn->m_user_id) {
        ...
        if (svr->m_debug) {
            CPE_INFO(
                ...
                conn_svr_name(svr), conn->m_conn_id, conn->m_fd, conn->m_user_id, conn->m_user_id);
        }
    }

    conn->m_user_id = user_id;
    ...
    if (conn->m_user_id) {
        ...
        if (cpe_hash_table_insert_unique(&svr->m_conns_by_user_id, conn) != 0) {
            CPE_ERROR(
                ...
                conn_svr_name(svr), conn->m_conn_id, conn->m_fd, conn->m_user_id, user_id);
            conn->m_user_id = 0;
            return -1;
        }
    ...
    while (blk == NULL) {
        ...
        if(collect_id < 0) {
            CPE_ERROR(
                ...
                conn_svr_name(svr), conn->m_conn_id, conn->m_fd, (int)size);
            conn_svr_conn_free(conn);
            return -1;
        }
        ...
        CPE_INFO(
            ...
            conn_svr_name(svr), conn->m_conn_id, conn->m_fd, free_conn->m_conn_id, free_conn->m_fd);
