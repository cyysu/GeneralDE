...
static void conn_cli_fsm_disable_enter(fsm_machine_t fsm, fsm_def_state_t state, void * event) {
...
static uint32_t conn_cli_fsm_disable_trans(fsm_machine_t fsm, fsm_def_state_t state, void * input_evt) {
...
int conn_cli_fsm_create_disable(fsm_def_machine_t fsm_def, error_monitor_t em) {
    fsm_def_state_t s = fsm_def_state_create_ex(fsm_def, "disable", conn_cli_state_disable);
    if (s == NULL) {
        CPE_ERROR(em, "%s: fsm_create_disable: create state fail!", fsm_def_machine_name(fsm_def));
        return -1;
    }

    fsm_def_state_set_action(s, conn_cli_fsm_disable_enter, NULL);

    if (fsm_def_state_add_transition(s, conn_cli_fsm_disable_trans) != 0) {
