...
conn_cli_svr_stub_t
conn_cli_svr_stub_create(conn_cli_t cli, const char * svr_type_name, uint16_t svr_type_id) {
    conn_cli_svr_stub_t svr;
    size_t name_len = strlen(svr_type_name) + 1;

    if (conn_cli_svr_stub_lsearch_by_name(cli, svr_type_name) != NULL) {
        CPE_ERROR(cli->m_em, "%s: create cli svr %s: name duplicate!", conn_cli_name(cli), svr_type_name);
        return NULL;
    }

    svr = mem_alloc(cli->m_alloc, sizeof(struct conn_cli_svr_stub) + name_len);
...
void conn_cli_svr_stub_free_all(conn_cli_t cli) {
    ...
    while(svr) {
        conn_cli_svr_stub_t next = cpe_hash_it_next(&svr_it);
        conn_cli_svr_stub_free(svr);
        svr = next;
    }
}

const char * conn_cli_svr_stub_type_name(conn_cli_svr_stub_t svr) {
    return svr->m_svr_type_name;
}

conn_cli_svr_stub_t conn_cli_svr_stub_lsearch_by_name(conn_cli_t cli, const char * name) {
    struct cpe_hash_it svr_it;
    conn_cli_svr_stub_t svr;
...
conn_cli_svr_stub_t conn_cli_svr_stub_find_by_id(conn_cli_t cli, uint16_t svr_type_id) {
    struct conn_cli_svr_stub key;
    key.m_svr_type_id = svr_type_id;
    return cpe_hash_table_find(&cli->m_svrs, &key);
}

cpe_hash_string_t conn_cli_svr_stub_response_dispatch_to(conn_cli_svr_stub_t svr) {
    return svr->m_response_dispatch_to;
}

int conn_cli_svr_stub_set_response_dispatch_to(struct conn_cli_svr_stub * svr, const char * response_dispatch_to) {
    conn_cli_t cli = svr->m_cli;
...
cpe_hash_string_t conn_cli_svr_stub_notify_dispatch_to(conn_cli_svr_stub_t svr) {
    return svr->m_notify_dispatch_to;
}

int conn_cli_svr_stub_set_notify_dispatch_to(struct conn_cli_svr_stub * svr, const char * notify_dispatch_to) {
    conn_cli_t cli = svr->m_cli;
...
int conn_cli_svr_stub_set_outgoing_recv_at(conn_cli_svr_stub_t svr, const char * outgoing_recv_at) {
    conn_cli_t cli = svr->m_cli;
    char name_buf[128];

    snprintf(name_buf, sizeof(name_buf), "%s.outgoing-recv-rsp", conn_cli_svr_stub_type_name(svr));
    ...
    dp_rsp_set_processor(svr->m_outgoing_recv_at, conn_cli_svr_stub_outgoing_recv, svr);

    if (dp_rsp_bind_string(svr->m_outgoing_recv_at, outgoing_recv_at, cli->m_em) != 0) {
        CPE_ERROR(
            cli->m_em, "%s: set outgoing_recv_at: bind to %s fail!",
            conn_cli_svr_stub_type_name(svr), outgoing_recv_at);
        dp_rsp_free(svr->m_outgoing_recv_at);
        svr->m_outgoing_recv_at = NULL;
        return -1;
    }
