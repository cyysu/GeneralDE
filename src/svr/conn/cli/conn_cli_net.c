...
static int conn_cli_process_data(conn_cli_t cli);
    ...
    if (revents & EV_READ) {
        ...
        if (blk == NULL) {
            CPE_ERROR(
                ...
                conn_cli_name(cli), (int)cli->m_read_block_size);
            ...
            if (bytes > 0) {
                if (cli->m_debug >= 2) {
                    CPE_INFO(
                        ...
                        conn_cli_name(cli), bytes);
                }

                ringbuffer_shrink(cli->m_ringbuf, blk, bytes);
                conn_cli_link_node_r(cli, blk);
                break;
            }
            else if (bytes == 0) {
                blk = ringbuffer_yield(cli->m_ringbuf, blk, cli->m_read_block_size);
                assert(blk == NULL);
                CPE_ERROR(cli->m_em, "%s: free for recv return 0!", conn_cli_name(cli));
                conn_cli_apply_evt(cli, conn_cli_fsm_evt_disconnected);
                return;
            }
            else {
        ...
        if (conn_cli_process_data(cli) != 0) return;
    }
        ...
        while(cli->m_wb) {
            ...
            if (bytes > 0) {
                ...
                CPE_ERROR(
                    ...
                    conn_cli_name(cli), err, cpe_sock_errstr(err));
...
static int conn_cli_decode_pkg_buf(conn_cli_t cli, LPDRMETA meta, void const * data, size_t data_len) {
...
RESIZE_AND_TRY_AGAIN:
    ...
    if (cli->m_tb == NULL) {
        CPE_ERROR(
            ...
            conn_cli_name(cli), (int)data_len, (int)curent_pkg_size);
        return -1;
    }
    cli->m_tb->id = 3;
            ...
            if (curent_pkg_size < cli->m_max_pkg_size) {
                ...
                if (cli->m_debug) {
                    CPE_INFO(
                        ...
                        conn_cli_name(cli), (int)curent_pkg_size, (int)data_len);
                }
...
static int conn_cli_process_data(conn_cli_t cli) {
    ...
    conn_cli_pkg_t cli_pkg = conn_cli_incoming_pkg(cli);
    int decode_result;

    if (cli_pkg == NULL) {
        CPE_ERROR(
            cli->m_em, "%s: cli_pkg alloc fail!!!", conn_cli_name(cli));
        conn_cli_apply_evt(cli, conn_cli_fsm_evt_disconnected);
        return -1;
    }
    ...
    while(cli->m_rb) {
        ...
        if (pkg_data_len < sizeof(SVR_CONN_NET_RES_HEAD)) {
            CPE_ERROR(
                ...
                conn_cli_name(cli), pkg_data_len);
            conn_cli_apply_evt(cli, conn_cli_fsm_evt_disconnected);
            return -1;
        }
        ...
        svr_stub = conn_cli_svr_stub_find_by_id(cli, cli_pkg->m_svr_type);
        if (svr_stub == NULL) {
            CPE_ERROR(cli->m_em, "%s: svr_stub %d is unknown!", conn_cli_name(cli), cli_pkg->m_svr_type);
            continue;
        }
            ...
            if (cli_pkg->m_sn) {
                ...
                if (svr_stub->m_response_dispatch_to == NULL) {
                    CPE_ERROR(
                        ...
                        conn_cli_name(cli), conn_cli_svr_stub_type_name(svr_stub), cli_pkg->m_sn, (int)pkg_data_len);
                }
                ...
                if (svr_stub->m_notify_dispatch_to == NULL) {
                    CPE_ERROR(
                        ...
                        conn_cli_name(cli), conn_cli_svr_stub_type_name(svr_stub), (int)pkg_data_len);
                }
