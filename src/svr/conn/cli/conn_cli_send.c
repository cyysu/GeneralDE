...
static int conn_cli_send_i(conn_cli_t cli, conn_cli_svr_stub_t svr_stub, uint32_t sn, LPDRMETA meta, void const * data, uint16_t data_len);

int conn_cli_send(conn_cli_t cli, uint16_t to_svr, uint32_t sn, LPDRMETA meta, void const * data, uint16_t data_len) {
    conn_cli_svr_stub_t svr_stub;

    svr_stub = conn_cli_svr_stub_find_by_id(cli, to_svr);
    if (svr_stub == NULL) {
        CPE_ERROR(cli->m_em, "%s: send: svr_stub %d is unknown!", conn_cli_name(cli), to_svr);
        return -1;
    }

    return conn_cli_send_i(cli, svr_stub, sn, meta, data, data_len);
}
...
static int conn_cli_send_i(conn_cli_t cli, conn_cli_svr_stub_t svr_stub, uint32_t sn, LPDRMETA meta, void const * data, uint16_t data_len) {
    ...
    if (meta && svr_stub->m_pkg_meta && meta != svr_stub->m_pkg_meta) {
        CPE_ERROR(
            ...
            conn_cli_name(cli), svr_stub->m_svr_type_name, svr_stub->m_svr_type_id,
...
RESIZE_AND_TRY_AGAIN:
    ...
    conn_cli_link_node_w(cli, blk);

    if (fsm_machine_curent_state(&cli->m_fsm) == conn_cli_state_established) {
        assert(cli->m_fd != -1);
...
int conn_cli_svr_stub_outgoing_recv(dp_req_t req, void * ctx, error_monitor_t em) {
    conn_cli_svr_stub_t svr_stub = ctx;
    conn_cli_t cli = svr_stub->m_cli;
    conn_cli_pkg_t pkg;

    pkg = conn_cli_pkg_find(req);
    if (pkg == NULL) {
        CPE_ERROR(cli->m_em, "%s: send: cast req to pkg fail!", conn_cli_name(cli));
        return -1;
    }

    return conn_cli_send_i(svr_stub->m_cli, svr_stub, pkg->m_sn, dp_req_meta(req), dp_req_data(req), dp_req_size(req));
}
