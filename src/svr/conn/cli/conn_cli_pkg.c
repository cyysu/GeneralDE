...
conn_cli_pkg_t
conn_cli_pkg_create(conn_cli_t cli) {
    dp_req_t dp_req;
    conn_cli_pkg_t pkg;

    dp_req = dp_req_create(gd_app_dp_mgr(cli->m_app), sizeof(struct conn_cli_pkg));
    if (dp_req == NULL) return NULL;

    dp_req_set_type(dp_req, req_type_conn_cli_pkg);

    pkg = (conn_cli_pkg_t)dp_req_data(dp_req);

    pkg->m_cli = cli;
    pkg->m_dp_req = dp_req;

    conn_cli_pkg_init(pkg);
...
conn_cli_t conn_cli_pkg_agent(conn_cli_pkg_t pkg) {
    return pkg->m_cli;
}
...
dp_req_t conn_cli_pkg_to_dp_req(conn_cli_pkg_t req) {
    return req->m_dp_req;
}

conn_cli_pkg_t conn_cli_pkg_find(dp_req_t req) {
    dp_req_t pkg = dp_req_child_find(req, req_type_conn_cli_pkg);
    return pkg ? (conn_cli_pkg_t)dp_req_data(pkg) : NULL;
}

const char * req_type_conn_cli_pkg = "conn_cli_pkg";
