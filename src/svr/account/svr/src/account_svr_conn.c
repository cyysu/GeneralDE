...
int account_svr_conn_bind_account(
    ...
    uint16_t conn_svr_id, uint16_t conn_svr_type, uint64_t account_id)
{
    dp_req_t pkg_buf;
    SVR_CONN_PKG * pkg;
    uint32_t conn_id;
    ...
    pkg->data.svr_conn_req_bind_user.conn_id = conn_id;
    pkg->data.svr_conn_req_bind_user.user_id = account_id;
