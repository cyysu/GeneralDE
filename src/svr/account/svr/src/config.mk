...
$(product).cpe-dr.pro.source:=$(addprefix $(product-base)/../pro/, \
                                   cli/svr_account_pro.xml \
                                   svr/svr_account_internal.xml)
$(product).cpe-dr.pro.h.output:=protocol/svr/account
$(product).cpe-dr.pro.c.output:=protocol/svr/account/metalib.c
...
$(product).cpe-dr.pro-use.source:=$(addprefix $(product-base)/../pro/, \
	                                   cli/svr_account_pro.xml)
$(product).cpe-dr.pro-use.bin.output:=meta/account_svr.dr

$(eval $(call product-def,$(product)))

product:=account_svr
$(product).type:=progn
$(product).depends:=account_svr_lib argtable2
