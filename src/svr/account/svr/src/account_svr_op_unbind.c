    ...
    logic_context_t ctx, logic_stack_node_t stack, void * user_data, cfg_t cfg)
{
    ...
    SVR_ACCOUNT_REQ_UNBIND * req;
    uint16_t from_svr_type;

    req_data = logic_context_data_find(ctx, "svr_account_req_unbind");
    if (req_data == NULL) {
        APP_CTX_ERROR(svr->m_app, "%s: unbind: get request fail!", account_svr_name(svr));
    ...
    if (set_logic_rsp_context_get_conn_info(ctx, &from_svr_type, NULL) != 0) {
        APP_CTX_ERROR(svr->m_app, "%s: unbind: get_carry_info fail!", account_svr_name(svr));
        logic_context_errno_set(ctx, SVR_ACCOUNT_ERROR_INTERNAL);
        return logic_op_exec_result_false;
    }
    ...
    if (account_svr_is_conn_svr(svr, from_svr_type)) {
        ...
        if (account_svr_conn_get_conn_info(svr, ctx, NULL, &account_id) != 0) {
            APP_CTX_ERROR(svr->m_app, "%s: unbind: get account_id from conn info fail!", account_svr_name(svr));
            logic_context_errno_set(ctx, SVR_ACCOUNT_ERROR_INTERNAL);
            return logic_op_exec_result_false;
        }
        ...
        if (req->account_id == 0) {
            ...
            if (req->account_id != account_id) {
                APP_CTX_ERROR(svr->m_app, "%s: unbind: can`t unbind to other account!", account_svr_name(svr));
    ...
    require = logic_require_create(stack, "unbind_query");
    if (require == NULL) {
        APP_CTX_ERROR(svr->m_app, "%s: unbind: create logic require fail!", account_svr_name(svr));
        logic_context_errno_set(ctx, SVR_ACCOUNT_ERROR_INTERNAL);
        return logic_op_exec_result_false;
    }

    if (account_svr_db_send_unbind(svr, require, req->account_id, req->account_type) != 0) {
        logic_context_errno_set(ctx, SVR_ACCOUNT_ERROR_INTERNAL);
    ...
    void * user_data, cfg_t cfg)
{
    ...
    if (logic_require_state(require) != logic_require_state_done) {
        if (logic_require_state(require) == logic_require_state_error) {
            APP_CTX_ERROR(
                svr->m_app, "%s: unbind: db request error, errno=%d!",
            ...
            APP_CTX_ERROR(
                svr->m_app, "%s: unbind: db request state error, state=%s!",
    ...
    if (db_op_res == NULL) {
        APP_CTX_ERROR(svr->m_app, "%s: unbind: find db op res fail!", account_svr_name(svr));
        logic_context_errno_set(ctx, SVR_ACCOUNT_ERROR_DB);
        return logic_op_exec_result_false;
    }
    ...
    if (updated_count != 1) {
        APP_CTX_ERROR(svr->m_app, "%s: unbind: updated %d records, error!", account_svr_name(svr), updated_count);
        logic_context_errno_set(ctx, SVR_ACCOUNT_ERROR_INTERNAL);
        return logic_op_exec_result_false;
    }
