...
int center_agent_center_send(struct center_agent_center * center, SVR_CENTER_PKG * pkg, size_t pkg_size) {
    net_ep_t ep = net_connector_ep(center->m_connector);
    size_t curent_pkg_size = mem_buffer_size(&center->m_outgoing_encode_buf);
    void * encode_buf;
    size_t buf_size;
    size_t input_size;
    ...
    do {
        if (mem_buffer_size(&center->m_outgoing_encode_buf) < curent_pkg_size) {
            if (mem_buffer_set_size(&center->m_outgoing_encode_buf, curent_pkg_size) != 0) {
                CPE_ERROR(
                    center->m_agent->m_em, "%s: center ep %d: send: set encode buf to size %d fail",
                    center_agent_name(center->m_agent), net_ep_id(ep),
                    (int)curent_pkg_size);
                return -1;
            }
        }

        encode_buf = mem_buffer_make_continuous(&center->m_outgoing_encode_buf, 0);
        assert(encode_buf);
        buf_size = mem_buffer_size(&center->m_outgoing_encode_buf);
        input_size = pkg_size;
        ...
        if (cvt_result == dr_cvt_result_not_enough_input) {
            if (curent_pkg_size >= center->m_max_pkg_size) {
                CPE_ERROR(
                    center->m_agent->m_em, "%s: center ep %d: send: not enough encode buf, buf size is %d!",
                    center_agent_name(center->m_agent), net_ep_id(ep),
                    (int)curent_pkg_size);
                return -1;
            }
            ...
            CPE_ERROR(
                center->m_agent->m_em, "%s: center ep %d: send: encode fail, buf size is %d!",
                center_agent_name(center->m_agent), net_ep_id(ep),
                (int)curent_pkg_size);
            return -1;
        }
        ...
        if (net_ep_send(ep, encode_buf, buf_size) != 0) {
            CPE_ERROR(
                center->m_agent->m_em, "%s: center ep %d: send: send to net fail, send-size=%d!",
                center_agent_name(center->m_agent), net_ep_id(ep),
                (int)buf_size);
            return -1;
        }
        ...
        if (center->m_agent->m_debug) {
            ...
            dr_json_print((write_stream_t)&stream, pkg, pkg_size, center->m_pkg_meta, 0, NULL);
            stream_putc((write_stream_t)&stream, 0);

            CPE_INFO(
                center->m_agent->m_em, "%s: center ep %d: ==> send one request, send-size=%d\n%s",
