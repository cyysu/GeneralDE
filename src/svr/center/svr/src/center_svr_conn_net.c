...
void center_svr_listener_cb(EV_P_ ev_io *w, int revents) {
    ...
    if (cpe_sock_set_none_block(new_fd, 1) != 0) {
        CPE_ERROR(
            svr->m_em, "%s: conn %d: set none block fail! errno=%d (%s)",
            center_svr_name(svr), new_fd, cpe_sock_errno(), cpe_sock_errstr(cpe_sock_errno()));
...
void center_svr_conn_rw_cb(EV_P_ ev_io *w, int revents) {
    ...
    if (revents & EV_READ) {
        ...
        if (blk == NULL) {
            CPE_ERROR(
                svr->m_em, "%s: conn %d: recv not enouth ringbuf, len=%d!",
                center_svr_name(svr), conn->m_fd, (int)svr->m_read_block_size);
        ...
        for(;;) {
            ...
            if (bytes > 0) {
                if (svr->m_debug >= 2) {
                    CPE_INFO(
                        svr->m_em, "%s: conn %d: recv %d bytes data!",
                        center_svr_name(svr), conn->m_fd, bytes);
                }
                ...
                assert(blk == NULL);
                CPE_ERROR(svr->m_em, "%s: conn %d: free for recv return 0!", center_svr_name(svr), conn->m_fd);
                center_svr_conn_free(conn);
                return;
            }
            else {
                ...
                default:
                    CPE_ERROR(
                        svr->m_em, "%s: conn %d: free for recv error, errno=%d (%s)!",
    ...
    if (revents & EV_WRITE) {
        while(conn->m_wb) {
            ...
            if (bytes > 0) {
                if (svr->m_debug >= 2) {
                    CPE_INFO(
                        svr->m_em, "%s: conn %d: send %d bytes data!",
                        center_svr_name(svr), conn->m_fd, bytes);
                }
                ...
                CPE_ERROR(
                    svr->m_em, "%s: conn %d: free for send return 0!",
                ...
                CPE_ERROR(
                    svr->m_em, "%s: conn %d: free for send error, errno=%d (%s)!",
                    center_svr_name(svr), conn->m_fd, err, cpe_sock_errstr(err));
    ...
    while(conn->m_rb) {
        ...
        if (pkg) {
            if (conn->m_svr->m_debug) {
                ...
                dr_json_print((write_stream_t)&stream, pkg, pkg_decode_len, svr->m_pkg_meta, 0, NULL);
                stream_putc((write_stream_t)&stream, 0);

                CPE_INFO(
                    conn->m_svr->m_em,
                    "%s: conn %d: <== recv one pkg (net-size=%d, data-size=%d)\n%s",
                    center_svr_name(svr), conn->m_fd, (int)pkg_data_len, (int)pkg_decode_len,
            ...
            {
                CPE_ERROR(
                    svr->m_em, "%s: conn %d: cmd %d: no processor of cmd!",
                    center_svr_name(svr), conn->m_fd, pkg->cmd);
                return 0;
            }
