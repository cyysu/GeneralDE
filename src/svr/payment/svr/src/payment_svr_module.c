...
int payment_svr_app_init(gd_app_context_t app, gd_app_module_t module, cfg_t cfg) {
    ...
    if (payment_svr_meta_product_info_load(payment_svr, cfg_find_cfg(gd_app_cfg(app), "meta.product_info")) != 0) {
        CPE_ERROR(gd_app_em(app), "%s: create: load product_info fail!", gd_app_module_name(module));
        payment_svr_free(payment_svr);
        return -1;
    }
