...
int dir_svr_app_init(gd_app_context_t app, gd_app_module_t module, cfg_t cfg) {
    ...
    const char * send_to;
    const char * recv_at;
    const char * str_game_id;
    uint16_t game_id;

    str_game_id = gd_app_arg_find(app, "--game-id");
    if (str_game_id == NULL) {
        CPE_ERROR(gd_app_em(app), "%s: create: game-id not configured in command line!", gd_app_module_name(module));
        return -1;
    }
    game_id = atoi(str_game_id);
    ...
    dir_svr = dir_svr_create(app, gd_app_module_name(module), game_id, stub, gd_app_alloc(app), gd_app_em(app));
    ...
    if (dir_svr->m_debug) {
        CPE_INFO(gd_app_em(app), "%s: create: done. game-id=%d", gd_app_module_name(module), game_id);
    }
...
static int dir_svr_load_regions(dir_svr_t svr) {
    ...
    while((region_cfg = cfg_it_next(&region_it))) {
        ...
        region = dir_svr_region_create(svr, region_id, region_name, region_state);
        if (region == NULL) {
            CPE_ERROR(
        ...
        if (svr->m_debug) {
            CPE_INFO(
                svr->m_em, "%s: load regions: %s: load region [id=%d, name=%s, state=%s]!",
